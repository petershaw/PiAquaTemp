...
module.exports = {
	getDataFromHours: function getDataFromHours(hours, callback){
		...
		console.log( "SELECT payload FROM aquatemp.by_hours WHERE log_time >= "+ from );
		client.execute(
			"SELECT payload FROM aquatemp.by_hours WHERE log_time >= ?"
			, [from]
			, cql.types.consistencies.any
			, function(err, result) {
				callback(err, result);
			}
		);
	...
	, getDataFromMinutes: function getDataFromMinutes(minutes, callback){
		...
		console.log( "SELECT probe_name, payload FROM aquatemp.by_minute WHERE log_time >= "+ from +" ALLOW FILTERING");
		client.execute(
