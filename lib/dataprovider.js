...
module.exports = {
	...
	, getDataFromMinutes: function getDataFromMinutes(minutes, callback){
		...
		var to   = new Date(moment(date).subtract(1, 'minutes')).getTime();
		client.execute(
			"SELECT log_time, probe_name, payload FROM aquatemp.by_minute WHERE log_time >= ? AND log_time <= ? ALLOW FILTERING"
			, [from, to]
			, cql.types.consistencies.one
			, function(err, result) {
				callback(err, result);
